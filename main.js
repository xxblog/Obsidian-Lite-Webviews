...
const MUTE_SCRIPT_MARKER = '__napMuteScriptV1';
const MUTE_SCRIPT =
...
const BG_FIX_MARKER = '__napBgFixStyle';
const BG_FIX_JS =
...
const BG_FIX_STYLE = '<script data-nap-bg-fix="' + BG_FIX_MARKER + '">' + BG_FIX_JS + '<\/script>';
const BG_FIX_CLEANUP_JS =
...
const DEFAULT_SETTINGS = {
	...
	defaultMute: true, // 默认静音：未单独设置卡片的初始静音状态
	otherCardTimeoutMs: 5 * 60 * 1000, // 操作其他卡片后 5 分钟挂起
	appBlurTimeoutMs: 30 * 1000, // 切走应用/标签页后 30 秒挂起
	buttonFontSizePx: 13, // 卡片按钮/状态文字的屏幕字号（固定像素，不随画布缩放变化）
	captureMinScreenPx: 100, // 抓图最小屏幕短边（px）；卡片太小不抓图，0 = 不限制
	screenshotQuality: 80, // 截图质量（%），10-100；100 使用 PNG 无损保存，<100 使用 JPEG
	fixTransparentBackground: true, // 修复部分网页在 Obsidian 中背景透明、透出控件的问题
	killRendererOnSuspend: false, // 挂起时杀渲染进程（实验性，彻底释放内存）
	cacheSizeLimitMB: 50, // 截图缓存大小上限（MB），0 = 不限制
};
...
class NoAutoplayPlugin extends Plugin {
	liveCards = new Set(); // 当前处于"真网页"状态的 webview（截图模式下）
	windows = new Set(); // 主窗口 + 全部 popout 窗口（样式/观察器/事件按窗口挂载）
	...
	sizeObservers = new Map(); // wv -> ResizeObserver（按钮尺寸随卡片缩放）
	scaleRoots = new WeakMap(); // wv -> 所在画布根容器（同根卡片共享缩放比例，轮询每根只读一次布局）
	removedElements = new Map(); // 父容器 -> {attrs, src}（挂起时被移除的 webview 快照）
	iframeDocs = new WeakMap(); // iframe -> {srcdoc, src}（Excalidraw 快照型嵌入的内容）
	iframeCaptureQueues = new WeakMap(); // iframe -> 串行抓图 Promise（避免同卡并发抓图）
	iframeGenerations = new WeakMap(); // iframe -> 内容代数（旧抓图完成时不再刷新占位）
	statusCleanups = new WeakMap(); // wv -> 清理"加载中…"状态监听器的函数
	muteHandlers = new WeakMap(); // wv -> reMute 监听器（卸载时移除，避免插件重载后残留旧监听）
	bgHandlers = new WeakMap(); // wv -> 背景修复监听器（卸载时移除）
	_unloaded = false;
	...
	effectiveScreenshotQuality() {
		const q = parseInt(this.settings.screenshotQuality, 10);
	...
	effectiveButtonSizePx() {
		const n = parseInt(this.settings.buttonFontSizePx, 10);
				...
				if (id) {
					try {
						const wc =
							...
							window.electron.remote.webContents &&
							window.electron.remote.webContents.fromId(id);
			...
			try {
				...
				for (const el of pending) {
					this.handle(el).catch((err) => {
						console.error('[no-autoplay] handle 失败:', err);
					});
				}
			} catch (e) {
				/* 观察器回调不抛未捕获异常 */
	...
	async onload() {
		...
		try {
			...
			console.error('[no-autoplay] 设置读取失败，使用默认设置:', e);
		}
		...
		this.addCommand({
			...
			callback: () => {
				const wv =
					this.lastActiveWv && this.lastActiveWv.isConnected
						? this.lastActiveWv
						: [...this.liveCards].find((w) => w.isConnected);
				if (!wv) {
					return;
				}
				const locked = wv.dataset.noAutoplayLocked === '1';
		...
		this.addSettingTab(new NoAutoplaySettingsTab(this.app, this));
	}
...
class NoAutoplaySettingsTab extends PluginSettingTab {
	...
	display() {
		try {
			...
			console.error('[no-autoplay] 设置页渲染失败:', e);
...
module.exports = NoAutoplayPlugin;
